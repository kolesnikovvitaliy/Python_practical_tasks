...
import time
aaaa = time.monotonic()
with open("data.json","r",encoding='utf8') as file_in, open("updated_data.json","w",encoding='utf8') as file_onn:
    data= json.loads(file_in.read())
    a = [eval(*[["i+'!'"],['i+1'],['not i'],['i*2'],['i | {"newkey": None}']][[str, int, bool, list, dict].index(type(i))]) for i in filter(lambda x: type(x)!=type(None),data)]
